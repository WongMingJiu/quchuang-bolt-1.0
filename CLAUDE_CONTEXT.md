...
- 已完成：
  ...
  - 提示词粘贴上传与 `@` 引用交互
  - 统一播放器与历史记录 / 资产页播放体验
  - 可用性标注功能（历史记录、资产页、下载拦截、标注持久化、筛选）
  - 管理中心入口、页面骨架、团队/个人/权限额度三个 tab 骨架
  - 管理中心相关数据库 migration 与后端接口骨架
  - 资产体系第一版重构（类型、数据库字段、资产页三大 tab、IP老师二级筛选、媒介筛选、创作资产标注边界）
  - 资产体系第一轮体验增强（资产详情抽屉信息分组、下载入口、资产所属类型可见性、非视频占位、筛选层次优化、创作页历史仅创作资产）
  - 提示词扩写三阶段后端工作流主骨架（输入解析 → 结构化补全 → 候选生成）
  - 提示词扩写本地轻量 API + Vite 定向代理链路打通
  - 线上 `prompt-expand` 路由可访问
  - 线上管理中心已上线“修复旧视频预览”按钮，可重刷已过期签名视频链接
  - 过期签名 `video_url` 已纳入修复范围，不再只修格式错误的 URL
  - Provider 任务已不存在的旧视频记录会被标记为“视频源已失效，无法恢复预览”，并在历史记录/资产页禁用播放
- 进行中：
  - 管理中心 mock 骨架的稳定维护
  - 提示词扩写输出质量继续向 `sd2-pe` 风格靠近
- 未开始：
  - 管理中心完整真实数据联调
  - 权限额度编辑真实保存
  - 个人 / 团队详情抽屉接真实接口
...
- 资产体系当前已完成第一版重构与第一轮体验增强：创作资产、参考素材、IP老师三大分类可见，IP老师包含形象 / 场景二级筛选，且可用性标注仅作用于创作资产，资产详情抽屉与筛选区域层次已优化。
- 管理中心当前是**前端可见 + 可交互骨架**，团队页、个人页、权限页都可见且主要按钮有交互，但仍以 **mock 数据为主**。
- 管理中心线上已补充“修复旧视频预览”运维入口，用于尝试刷新历史视频记录中的过期签名 URL。
- 历史视频当前存在两类状态：
  - provider 任务仍可查询：可通过修复按钮回写新的可播放链接
  - provider 任务已不存在：会被标记为“视频源已失效，无法恢复预览”，前端保留记录但禁用播放
- 提示词扩写当前已打通两条运行路径：
  - 线上通过 Vercel Functions 的 `/api/prompt-expand`
  - 本地通过“轻量 prompt API + Vite 仅代理 `/api/prompt-expand`”模式
...
# 当前任务
提示词扩写优化：当前三阶段工作流主骨架与本地/线上链路已打通，下一步重点转向 Step 1 / Step 2 / Step 3 的输出质量优化，以及进一步向官方 `sd2-pe` 风格靠近。

补充说明：线上历史视频预览问题本轮已完成止血处理——可恢复记录可重新刷新链接，不可恢复记录明确禁播并提示源已失效；后续如继续推进，应转向“如何避免未来继续存储会过期的 provider 签名 URL”。

# 下一步计划
1. 保留当前前端可见的扩写区与候选覆盖交互
2. 继续优化三阶段工作流的输出质量与候选差异化
3. 在不扩散范围的前提下，逐步提升线上与本地的结果一致性
...
- 历史视频预览当前依赖 provider 返回的签名 URL；一旦链接过期且 provider 任务已删除，记录只能被标记为不可恢复，无法重新拿回原视频。
- 线上“修复旧视频预览”属于止血型运维入口，不等于从根上解决 URL 生命周期问题。
