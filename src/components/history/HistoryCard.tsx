    ...
    <div className="card overflow-hidden group transition-all duration-200 hover:shadow-elevated hover:border-[#B6E7FF] animate-fade-in">
      ...
      <div className="p-3">
        ...
        <div className="flex items-center gap-1.5 mb-3 flex-wrap">
          ...
          <span className="tag text-[10px]">{g.model}</span>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-[#E6EDF5]">
          <div className="flex items-center gap-0.5">
            <button
              onClick={() => onRefill(g)}
              title="重新编辑"
              className="flex items-center gap-1 px-2 py-1.5 rounded text-xs text-[#475569] hover:bg-[#EAF3FF] hover:text-[#1F8BFF] transition-all duration-150 font-medium"
            >
              <Edit3 size={13} />
          ...
          <div className="flex items-center gap-0.5">
            <button
              onClick={() => onToggleFavorite(g.id, g.is_favorited)}
              title={g.is_favorited ? '取消收藏' : '收藏'}
            ...
            >
              <Star size={14} className={g.is_favorited ? 'fill-current' : ''} />
            </button>
            ...
            {g.status === 'completed' && g.video_url && (
              <a
                ...
                download
                title="下载"
                className="p-1.5 rounded text-[#94A3B8] hover:text-[#1F8BFF] hover:bg-[#EAF3FF] transition-all duration-150"
              >
                <Download size={14} />
              </a>
            )}
            ...
            <div className="relative">
              <button
                ...
                className="p-1.5 rounded text-[#94A3B8] hover:text-[#EF4444] hover:bg-[#FEF2F2] transition-all duration-150"
              >
                <Trash2 size={14} />
              </button>

              {showDelete && (
                <div className="absolute bottom-8 right-0 bg-white rounded-lg shadow-elevated border border-[#E6EDF5] p-3 z-20 w-48 animate-fade-in">
                  <p className="text-xs text-[#0F172A] font-medium mb-2">确认删除这条记录？</p>
                  <div className="flex gap-2">
                    <button
                      onClick={() => { onDelete(g.id); setShowDelete(false); }}
                      className="flex-1 px-2 py-1.5 bg-[#EF4444] text-white text-xs rounded font-medium hover:bg-red-600 transition-colors"
                    >
                      删除
                    </button>
                    <button
                      onClick={() => setShowDelete(false)}
                      className="flex-1 px-2 py-1.5 bg-[#EEF4FA] text-[#475569] text-xs rounded font-medium hover:bg-[#D8E2F0] transition-colors"
                    >
                      取消
                    </button>
                  </div>
