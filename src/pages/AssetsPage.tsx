...
function AssetCard({ generation: g, onToggleFavorite, onPreview }: AssetCardProps) {
          ...
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 gap-4">
              ...
              <div className="text-center">
                <p className="text-base font-semibold text-[#475569]">{favOnly ? '暂无收藏内容' : '暂无资产'}</p>
                <p className="text-sm text-[#94A3B8] mt-1">{favOnly ? '收藏生成结果后将显示在这里' : '生成完成的内容将自动归档到这里'}</p>
              </div>
              {favOnly && (
                <button onClick={() => setFavOnly(false)} className="text-sm text-[#1F8BFF] hover:text-[#4AA3FF] transition-colors">查看全部资产</button>
              )}
            </div>
          ) : (
                ...
                <div key={date}>
                  ...
                  <div style={{ columns: '3 240px', gap: '12px' }}>
                    {items.map((g: Generation) => (
                      <AssetCard key={g.id} generation={g} onToggleFavorite={onToggleFavorite} onPreview={setPreviewTarget} />
                    ))}
                  </div>
                </div>
              ))}
